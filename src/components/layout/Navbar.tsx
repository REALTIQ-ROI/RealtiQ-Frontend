...
const Navbar = () => {
  ...
  const cartPath = isAuthenticated && user?.role === 'buyer' ? '/dashboard/buyer/cart' : '/cart';
  const canUseCart = !isAuthenticated || user?.role === 'buyer';
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 sm:py-4">
        ...
        {isAuthenticated ? (
          <div className="hidden items-center gap-3 md:flex">
            {canUseCart ? (
              <Link className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-surface-container-low" to={cartPath} aria-label="Service cart">
      ...
      {open ? (
        <div className="border-t border-outline-variant/20 bg-surface px-4 py-4 md:hidden" key={location.pathname}>
          <div className="flex flex-col gap-1">
            ...
            {canUseCart ? (
              <Link to={cartPath} onClick={() => setOpen(false)} className="rounded-lg px-4 py-3 text-sm font-bold text-on-surface-variant">
