...
const InspectorOnboarding = () => {
  ...
  const kyc = user?.kyc; const status = kyc?.status || (user?.emailVerified ? 'kyc_pending' : 'registration_pending');
  const canSubmit = user?.emailVerified && ['kyc_pending', 'pending', 'rejected'].includes(status);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setValues({ fullLegalName: '', phone: '', address: '', nationalId: '', labels: '' }); setIdDocument(undefined); setSelfie(undefined); setDocuments([]);
  ...
  return <ProxyInspectorLayout title="Verification and onboarding"><section className="rounded-2xl bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-widest text-secondary">Current status</p><h2 className="mt-2 text-3xl font-black capitalize">{status.replaceAll('_', ' ')}</h2>
    ...
    {status === 'approved' ? <p className="mt-3 text-emerald-800">Your professional profile is approved. Public visibility and availability remain backend-controlled.</p> : null}
  ...
  {canSubmit ? <form className="mt-6 space-y-5 rounded-2xl bg-white p-6 shadow-sm" onSubmit={(e) => void submit(e)} autoComplete="off"><h2 className="text-2xl font-black">Secure KYC submission</h2><p className="text-sm text-secondary">These details are sent directly to the backend and are not saved in browser storage. Uploaded credentials are unverified until Admin review.</p>
